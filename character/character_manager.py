...
class _HasStatCible(Protocol):
    """Interface minimale d'un objet progresseur (UpStat / UpStatCollection)."""
    stat_cible: str
    stat_min:   float
    stat_max:   float
...
class CharacterManager:
    ...
    def _resolve_save_file(self) -> str:
        """Retourne le chemin du fichier de sauvegarde, migre depuis l'ancien si besoin."""
    ...
    def _flush_save(self) -> None:
        """Sauvegarde si des stats ont changé depuis le dernier flush."""
    ...
    def stop_up(self) -> None:
        self.up = False
        self._flush_save()   # sauvegarde dès que le joueur quitte l'objet
    ...
    def update_player_stats(self, delta_time: float = 1 / 60) -> None:
        ...
        # Auto-save périodique pendant la lecture (toutes les _AUTO_SAVE_INTERVAL secondes)
        self._auto_save_timer += delta_time
        if self._auto_save_timer >= _AUTO_SAVE_INTERVAL:
            self._flush_save()

        # Incrément de stat toutes les up_increase_interval secondes
        ...
        self._stats_dirty = True   # marqué modifié — sauvegarde différée
