...
class InteractUI:
    ...
    def interact_obj_prg(self) -> None:
        ...
        # Passe 0 : ObjetInteractif — popup avec nom fixe
        for objet in gv.objet_sprites:
            if not isinstance(objet, ObjetInteractif):
                continue
        ...
        for objet in gv.objet_sprites:
            if not isinstance(objet, MapActionObject):
                continue
            if arcade.get_distance_between_sprites(player, objet) >= 68:
                continue
        ...
        for objet in gv.objet_sprites:
            ...
            if isinstance(objet, UpStatCollection):
